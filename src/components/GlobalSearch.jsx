...
export default function GlobalSearch({ buttonClassName = '' }) {
  ...
  const results = useMemo(() => {
    ...
    const people = currentUser.role === 'manager' || currentUser.role === 'admin' ? users.filter((user) => user.role === 'employee').map((user) => ({ type: 'עובד', icon: Users, title: user.username, subtitle: user.department, to: '/manager' })) : [];
  ...
  return <><button type="button" onClick={() => setOpen(true)} className={buttonClassName} aria-label="חיפוש גלובלי"><Search size={18} /><span className="hidden xl:inline">חיפוש</span><kbd className="hidden rounded border border-gray-700 px-1.5 py-0.5 text-[9px] text-gray-600 lg:inline">⌘K</kbd></button>{open && <div className="fixed inset-0 z-[120] flex items-start justify-center bg-black/75 p-4 pt-[12vh] backdrop-blur-sm" onMouseDown={() => setOpen(false)}><section className="w-full max-w-2xl overflow-hidden rounded-3xl border border-gray-700 bg-[#0b0f19] shadow-2xl" onMouseDown={(event) => event.stopPropagation()} dir="rtl"><div className="flex items-center gap-3 border-b border-gray-800 p-4"><Search className="text-[#00e6ff]" /><input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} className="w-full bg-transparent text-base text-white outline-none" placeholder="חיפוש עובדים, קורסים, מבחנים, סרטונים ומסמכים…" /><button onClick={() => setOpen(false)} className="text-gray-500"><X /></button></div><div className="max-h-[55vh] overflow-y-auto p-3">{query && !results.length && <p className="p-8 text-center text-xs font-bold text-gray-600">לא נמצאו תוצאות</p>}{results.map((item, index) => { const Icon = item.icon; return <Link key={`${item.type}-${item.title}-${index}`} to={item.to} onClick={() => { setOpen(false); setQuery(''); }} className="flex items-center gap-3 rounded-2xl p-3 hover:bg-gray-900"><span className="grid h-9 w-9 place-items-center rounded-xl bg-[#00e6ff]/8 text-[#00e6ff]"><Icon size={17} /></span><span className="flex-1"><strong className="block text-xs text-white">{item.title}</strong><small className="mt-1 block text-[10px] text-gray-600">{item.subtitle}</small></span><span className="rounded bg-gray-900 px-2 py-1 text-[9px] font-bold text-gray-500">{item.type}</span></Link>; })}</div></section></div>}</>;
}
